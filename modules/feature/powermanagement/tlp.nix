...
let
  cfg = config.host.feature.powermanagement.tlp;
  MHz = x: x * 1000;
in
  with lib;
{
  options = {
    host.feature.powermanagement.tlp = {
      enable = mkOption {
        default = false;
        type = with types; bool;
        description = "Enables utilzing TLP (Thinkpad Laptop Power) management";
      };
    };
  };
